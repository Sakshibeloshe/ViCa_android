...
class CardRepository(private val cardDao: CardDao) {
    ...
    fun getMyCards() = cardDao.getMyCardsWithFields()
    fun getReceivedCards() = cardDao.getReceivedCardsWithFields()
    fun getAllCards() = cardDao.getAllCardsWithFields()

    suspend fun getCardById(cardId: UUID) = cardDao.getCardWithFieldsById(cardId)
    ...
    /**
     * Persist a card received via NFC tap-to-transfer.
     * Inserts the decoded [CardModel] directly without going through FieldCatalog.
     */
    suspend fun insertReceivedCard(card: CardModel) {
        val entity = CardEntity(
            ...
            photoData   = null,           // Photo arrives later via BLE
        ...
        val fields = card.fields.mapIndexed { index, field ->
            FieldEntity(
                id         = UUID.randomUUID(),
                cardId     = card.id,
                key        = field.key,
        ...
        cardDao.insertCardWithFields(entity, fields)
    }
}
